"""
train_linear_probe.py — Fase 2: Frozen backbone + trainable linear head.

Usage:
    python -m src.training.train_linear_probe --config configs/train_linear_probe.yaml

Backbone MAE-ViT dibekukan total, hanya classification head yang dilatih.
Ini adalah baseline bawah untuk perbandingan performa.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train linear probe (frozen backbone)")
    parser.add_argument("--config", type=str, default="configs/train_linear_probe.yaml")
    ...
    # Simpan snapshot pretrained (PENTING untuk analisis T2/T3)
    ...
    print(f"\n[linear_probe] Total params: {total_params:,}")
    print(f"[linear_probe] Trainable params: {trainable_params:,} "
    ...
    print(f"\n[linear_probe] Results saved to {results_path}")
    print("[linear_probe] Done.")
